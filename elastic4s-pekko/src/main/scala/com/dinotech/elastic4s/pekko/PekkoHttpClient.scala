...
import org.apache.pekko.stream.{ActorMaterializer, OverflowStrategy, QueueOfferResult}
...
class PekkoHttpClient private[pekko] (settings: PekkoHttpClientSettings, blacklist: Blacklist, httpPoolFactory: HttpPoolFactory)(implicit
                                                                                                                              system: ActorSystem
  ...
  private[pekko] def sendAsync(request: ElasticRequest): Future[ElasticHttpResponse] =
...
object PekkoHttpClient {
  ...
  def apply(settings: PekkoHttpClientSettings)(implicit system: ActorSystem): PekkoHttpClient = {
  ...
  private[pekko] case class RequestState(response: Promise[ElasticHttpResponse] = Promise(), host: Promise[String] = Promise())

  private[pekko] case object AllHostsBlacklistedException extends Exception("All hosts are blacklisted!")
